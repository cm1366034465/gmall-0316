...
public class ItemService {
    ...
    public ItemVo loadData(Long skuId) {
        ...
        CompletableFuture<Void> categoryCompletableFuture = skuCompletableFuture.thenAcceptAsync(skuEntity -> {
            // 根据sku中的categoryId查询一二三分类
            ResponseVo<List<CategoryEntity>> listResponseVo = this.pmsClient.query123CategoriesByCid3(skuEntity.getCatagoryId());
        ...
        CompletableFuture<Void> skuSaleCompletableFuture = CompletableFuture.runAsync(() -> {
            // 根据skuId 查询当前sku的销售属性
            ResponseVo<List<SkuAttrValueEntity>> skuSaleAttr = this.pmsClient.querySaleAttrValueBySkuId(skuId);
            List<SkuAttrValueEntity> skuAttrValueEntities = skuSaleAttr.getData();
        ...
        CompletableFuture<Void> groupCompletableFuture = skuCompletableFuture.thenAcceptAsync(skuEntity -> {
            // 根据sku的分类id、spuId、skuId 查询规格参数和值
